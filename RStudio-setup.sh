...
chown $(id -u):$(id -g) /opt/anaconda3/ -R
...
sudo gdebi rstudio-server-1.2.1335-amd64.deb # First try will fail because we haven't told RStudio-Server which R to use. We start Server again below
...
# Restart terminal, then start RStudio-Server:
